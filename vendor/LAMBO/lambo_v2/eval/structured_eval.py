...
def evaluate_prediction_against_gold(prediction: Any, gold_answer: Any) -> Dict[str, Any]:
    ...
    if isinstance(gold_answer, list) and isinstance(prediction, list):
        normalized_prediction = normalize_sequence_values(prediction)
        normalized_gold = normalize_sequence_values(gold_answer)
        ...
        position_accuracy = position_hits / len(normalized_gold) if normalized_gold else 1.0
        return {
            "exact_match": normalized_prediction == normalized_gold,
            "pair_precision": precision,
            "pair_recall": recall,
...
def evaluate_predictions(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for row in rows:
        prediction = row.get("judge_prediction", row.get("generate_response"))
